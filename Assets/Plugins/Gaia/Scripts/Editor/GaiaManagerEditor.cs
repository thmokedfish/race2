...
namespace Gaia
{
    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        GameObject CreatePlayer()
        {
            ...
            GameObject flyCam = GameObject.Find("FlyCam");
            GameObject thirdController = GameObject.Find("ThirdPersonController");
            GameObject flyCamUI = GameObject.Find("FlyCamera UI");

            if (fpsController != null)
            {
                DestroyImmediate(fpsController);
            }
            ...
            if (flyCam != null)
            {
                DestroyImmediate(flyCam);
            }

            if (thirdController != null)
            {               
                DestroyImmediate(thirdController);
            }

            if (flyCamUI != null)
            {
                DestroyImmediate(flyCamUI);
            }
            ...
            if (playerPrefabName == "Flycam")
            {
                ...
                playerObj.GetComponent<Camera>().farClipPlane = theTerrain.terrainData.size.x / 2 + 300f;
                location.y += 1.8f;
                playerObj.transform.position = location;

                string flyCameraUI = GetAssetPath("FlyCamera UI");
                GameObject theUI = Instantiate(AssetDatabase.LoadAssetAtPath<GameObject>(flyCameraUI));
                theUI.name = "FlyCamera UI";
                theUI.transform.SetParent(playerObj.transform);
            ...
            {
                GameObject fps = GaiaCommon1.AssetUtils.GetAssetPrefab(playerPrefabName);
                if (fps != null)
                {
                    location.y += 1f;
        ...
        public void ImportDependsOnGaiaStartUp()
        {
            ...
            string postProcessing = GetAssetPath("PostProcessing-2");

            if (string.IsNullOrEmpty(standardAssets) || string.IsNullOrEmpty(speedTreesB) || string.IsNullOrEmpty(speedTreesC) || string.IsNullOrEmpty(speedTreesP) || string.IsNullOrEmpty(postProcessing))
            {
                if (EditorUtility.DisplayDialog(
                    m_editorUtils.GetTextValue("OOPSMISSING!"),
                    m_editorUtils.GetTextValue("Missing Depends"),
